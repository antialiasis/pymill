...
class Pymill(object):
    ...
    def _api_call(self, url, params=None, method="GET", force_get=False, headers=None, parse_json=True, return_type=None):
        """Call an API method.
        ...
        if len(params) > 0 and not method in ('DELETE', 'PUT') and not force_get:
    ...
    def get_transactions(self, filters=None):
        ...
        """
        return self._api_call("https://api.paymill.com/v2/transactions/", params=filters, force_get=True, return_type=Transaction)
    ...
    def update_subscription(self, subscription_id, offer=None, payment=None):
        """Change the offer or payment method that a subscription is attached to
        
        :Parameters:
         - `subscription_id` - ID of the subscription
         - `offer` - ID of the new offer
         - `payment` - ID of the new payment information

        :Returns:
            a dict with a member "data" which represents the subscription
        """
        return self._api_call("https://api.paymill.de/v2/subscriptions/" + str(subscription_id), dict_without_none(offer=str(offer), payment=str(payment)), method="PUT", return_type=Subscription)
